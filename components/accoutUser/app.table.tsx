...
export function DataUserTable() {
  ...
  const [apiData, setApiData] = React.useState<IUser[]>([]);
  const [userOption, setUserOption] = React.useState<IUser | null>(null);
  const [selectedUserOptionId, setSelectedUserOptionId] = React.useState<
    number | null
  >(null);
  // Hàm tìm kiếm
  ...
  const getAllUsers = async () => {
    const api = "/auth/LayTatCaThongTinUser";
    try {
      setIsLoading(true);
      ...
      if (res) {
        ...
        console.log(`User list option not found`);
      }
    } catch (error) {
            ...
            <Dialog>
              <DialogTrigger asChild>
                <Button
                  variant="secondary"
                  className="mx-3"
                  onClick={() => setSelectedUserOptionId(user.userID)}
                >
